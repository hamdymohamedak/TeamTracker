...
export function hashBytesFnv1a(data) {
    let h = 0x811c9dc5;
 ...
 */
export function frameSignatureFromRgba(pixels, width, height, channels = 4) {
...
export function frameSignatureFromBuffer(buf) {
    if (buf.length === 0)
        return 0;
...
export function createUnchangedThrottleState() {
 ...
 */
export function shouldSendFrame(state, hash, nowMs, forceRefreshMs = 4000) {
