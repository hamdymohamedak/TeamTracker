...
export const LIVE_VIEW_DEFAULT_MAX_FRAME_BYTES = 220000;
function uuidToBytes(sessionId) {
...
export function encodeLiveViewBinaryFrame(input) {
    ...
    const flags = (input.privacyBlocked ? LIVE_VIEW_FLAG_PRIVACY_BLOCKED : 0) |
    ...
    if (jpeg.length)
        buf.set(jpeg, LIVE_VIEW_BINARY_HEADER_SIZE);
    return buf;
}
export function peekLiveViewBinaryMagic(data) {
    if (data.byteLength < 4)
        return false;
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    return view.getUint32(0) === LIVE_VIEW_BINARY_MAGIC;
}
export function decodeLiveViewBinaryFrame(data, maxFrameBytes = LIVE_VIEW_DEFAULT_MAX_FRAME_BYTES) {
...
export function peekBinarySessionId(data) {
    if (data.byteLength < LIVE_VIEW_BINARY_HEADER_SIZE)
        return null;
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    if (view.getUint32(0) !== LIVE_VIEW_BINARY_MAGIC)
...
export function peekBinaryPayloadLength(data) {
    if (data.byteLength < LIVE_VIEW_BINARY_HEADER_SIZE)
        return null;
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    if (view.getUint32(0) !== LIVE_VIEW_BINARY_MAGIC)
