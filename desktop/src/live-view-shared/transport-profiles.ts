/**
 * Transport-aware Live View performance profiles.
 * LAN high FPS only applies on WebRTC P2P; Binary WS stays VPS-safe.
 * LAN detection is a performance signal — never a security boundary.
 */

import {
  LIVE_VIEW_QUALITY_PRESETS,
  type LiveViewEncodeLevel,
  type LiveViewQualityPreset,
} from './quality.js';

export type LiveViewNetworkPath =
  | 'webrtc-p2p-lan'
  | 'webrtc-p2p-internet'
  | 'webrtc-turn'
  | 'binary-ws'
  | 'unknown';

export type IceCandidateType = 'host' | 'srflx' | 'prflx' | 'relay' | 'unknown';

export interface LiveViewProfileConfig {
  /** Binary WS hard FPS cap (LIVE_VIEW_MAX_FPS). */
  wsMaxFps: number;
  wsMinFps: number;
  lanMaxFps: number;
  lanDefaultFps: number;
  internetMaxFps: number;
  turnMaxFps: number;
  /** RTT (ms) at or below which host/prflx P2P counts as LAN. */
  lanRttMaxMs: number;
}

export const DEFAULT_LIVE_VIEW_PROFILE_CONFIG: LiveViewProfileConfig = {
  wsMaxFps: 4,
  wsMinFps: 1,
  lanMaxFps: 30,
  lanDefaultFps: 12,
  internetMaxFps: 4,
  turnMaxFps: 4,
  lanRttMaxMs: 40,
};

export interface EffectiveLiveViewPreset {
  level: LiveViewEncodeLevel;
  width: number;
  jpegQuality: number;
  fps: number;
  intervalMs: number;
  maxBitrateBps: number;
  networkPath: LiveViewNetworkPath;
}

/** Discrete FPS choices exposed in the Dashboard per path. */
export const LAN_FPS_OPTIONS = [5, 10, 12, 15, 20, 30] as const;
export const INTERNET_FPS_OPTIONS = [1, 2, 3, 4] as const;

/** LAN Auto: raise FPS at 720p before bumping resolution. */
export const LAN_AUTO_FPS_LADDER = [5, 10, 12, 15] as const;

interface PathLevelSpec {
  width: number;
  fps: number;
  jpegQuality: number;
  maxBitrateBps: number;
}

const LAN_LEVEL_SPECS: Record<LiveViewEncodeLevel, PathLevelSpec> = {
  low: { width: 720, fps: 5, jpegQuality: 32, maxBitrateBps: 800_000 },
  medium: { width: 720, fps: 12, jpegQuality: 36, maxBitrateBps: 2_000_000 },
  high: { width: 1080, fps: 18, jpegQuality: 42, maxBitrateBps: 4_000_000 },
  ultra: { width: 1080, fps: 30, jpegQuality: 48, maxBitrateBps: 6_000_000 },
};

function internetSpec(level: LiveViewEncodeLevel): PathLevelSpec {
  if (level === 'ultra') {
    const high = LIVE_VIEW_QUALITY_PRESETS.high;
    return {
      width: high.width,
      fps: high.fps,
      jpegQuality: high.jpegQuality,
      maxBitrateBps: high.maxBitrateBps,
    };
  }
  const p = LIVE_VIEW_QUALITY_PRESETS[level];
  return {
    width: p.width,
    fps: p.fps,
    jpegQuality: p.jpegQuality,
    maxBitrateBps: p.maxBitrateBps,
  };
}

export function mergeProfileConfig(
  partial?: Partial<LiveViewProfileConfig>
): LiveViewProfileConfig {
  return { ...DEFAULT_LIVE_VIEW_PROFILE_CONFIG, ...partial };
}

export function isLiveViewNetworkPath(value: unknown): value is LiveViewNetworkPath {
  return (
    value === 'webrtc-p2p-lan' ||
    value === 'webrtc-p2p-internet' ||
    value === 'webrtc-turn' ||
    value === 'binary-ws' ||
    value === 'unknown'
  );
}

export function parseIceCandidateType(value: unknown): IceCandidateType {
  if (value === 'host' || value === 'srflx' || value === 'prflx' || value === 'relay') {
    return value;
  }
  return 'unknown';
}

function isLanCandidateType(t: IceCandidateType): boolean {
  return t === 'host' || t === 'prflx';
}

export interface IcePathSample {
  localType: IceCandidateType;
  remoteType: IceCandidateType;
  rttMs: number | null;
}

/**
 * Classify WebRTC route from selected ICE candidate pair.
 * Relay → TURN. host/prflx + low RTT → LAN. Otherwise Internet P2P.
 */
export function classifyIcePath(
  sample: IcePathSample,
  lanRttMaxMs: number = DEFAULT_LIVE_VIEW_PROFILE_CONFIG.lanRttMaxMs
): Exclude<LiveViewNetworkPath, 'binary-ws' | 'unknown'> {
  if (sample.localType === 'relay' || sample.remoteType === 'relay') {
    return 'webrtc-turn';
  }
  const lanEligible =
    isLanCandidateType(sample.localType) && isLanCandidateType(sample.remoteType);
  const rttOk = sample.rttMs == null || sample.rttMs <= lanRttMaxMs;
  if (lanEligible && rttOk) return 'webrtc-p2p-lan';
  return 'webrtc-p2p-internet';
}

/**
 * Inspect RTCStatsReport-like map (browser getStats() result).
 * Accepts Map or iterable of [id, report] / forEach-capable objects.
 */
export function detectNetworkPathFromStats(
  stats: { forEach: (cb: (report: Record<string, unknown>, id: string) => void) => void },
  lanRttMaxMs: number = DEFAULT_LIVE_VIEW_PROFILE_CONFIG.lanRttMaxMs
): {
  path: Exclude<LiveViewNetworkPath, 'binary-ws'>;
  localType: IceCandidateType;
  remoteType: IceCandidateType;
  rttMs: number | null;
  pairId: string | null;
} {
  const byId = new Map<string, Record<string, unknown>>();
  let selectedPairId: string | undefined;

  stats.forEach((report, id) => {
    byId.set(id, report);
    if (report.type === 'transport' && typeof report.selectedCandidatePairId === 'string') {
      selectedPairId = report.selectedCandidatePairId;
    }
  });

  let pair: Record<string, unknown> | null = null;
  if (selectedPairId && byId.has(selectedPairId)) {
    pair = byId.get(selectedPairId)!;
  } else {
    for (const report of byId.values()) {
      if (report.type === 'candidate-pair' && report.state === 'succeeded') {
        pair = report;
        break;
      }
    }
  }

  if (!pair) {
    return {
      path: 'unknown',
      localType: 'unknown',
      remoteType: 'unknown',
      rttMs: null,
      pairId: null,
    };
  }

  const local = pair.localCandidateId
    ? byId.get(String(pair.localCandidateId))
    : undefined;
  const remote = pair.remoteCandidateId
    ? byId.get(String(pair.remoteCandidateId))
    : undefined;

  const localType = parseIceCandidateType(local?.candidateType);
  const remoteType = parseIceCandidateType(remote?.candidateType);
  const rttMs =
    typeof pair.currentRoundTripTime === 'number'
      ? pair.currentRoundTripTime * 1000
      : null;

  const path = classifyIcePath({ localType, remoteType, rttMs }, lanRttMaxMs);
  return {
    path,
    localType,
    remoteType,
    rttMs,
    pairId: typeof pair.id === 'string' ? pair.id : selectedPairId || null,
  };
}

export function maxFpsForPath(
  path: LiveViewNetworkPath,
  cfg: LiveViewProfileConfig = DEFAULT_LIVE_VIEW_PROFILE_CONFIG
): number {
  switch (path) {
    case 'webrtc-p2p-lan':
      return cfg.lanMaxFps;
    case 'webrtc-turn':
      return cfg.turnMaxFps;
    case 'binary-ws':
      return cfg.wsMaxFps;
    case 'webrtc-p2p-internet':
    case 'unknown':
    default:
      return cfg.internetMaxFps;
  }
}

export function minFpsForPath(
  path: LiveViewNetworkPath,
  cfg: LiveViewProfileConfig = DEFAULT_LIVE_VIEW_PROFILE_CONFIG
): number {
  if (path === 'binary-ws') return cfg.wsMinFps;
  return path === 'webrtc-p2p-lan' ? 5 : cfg.wsMinFps;
}

export function allowedFpsOptions(
  path: LiveViewNetworkPath,
  cfg: LiveViewProfileConfig = DEFAULT_LIVE_VIEW_PROFILE_CONFIG
): number[] {
  const max = maxFpsForPath(path, cfg);
  const min = minFpsForPath(path, cfg);
  const base =
    path === 'webrtc-p2p-lan'
      ? [...LAN_FPS_OPTIONS]
      : [...INTERNET_FPS_OPTIONS];
  return base.filter((f) => f >= min && f <= max);
}

export function clampFpsForPath(
  fps: number,
  path: LiveViewNetworkPath,
  cfg: LiveViewProfileConfig = DEFAULT_LIVE_VIEW_PROFILE_CONFIG
): number {
  const options = allowedFpsOptions(path, cfg);
  if (options.length === 0) return maxFpsForPath(path, cfg);
  let best = options[0];
  let bestDist = Math.abs(fps - best);
  for (const opt of options) {
    const d = Math.abs(fps - opt);
    if (d < bestDist) {
      best = opt;
      bestDist = d;
    }
  }
  return best;
}

/** Highest encode level allowed on this path (Ultra is LAN-only). */
export function maxEncodeLevelForPath(path: LiveViewNetworkPath): LiveViewEncodeLevel {
  return path === 'webrtc-p2p-lan' ? 'ultra' : 'high';
}

export function isUltraAllowed(path: LiveViewNetworkPath): boolean {
  return path === 'webrtc-p2p-lan';
}

function specForPath(level: LiveViewEncodeLevel, path: LiveViewNetworkPath): PathLevelSpec {
  if (path === 'webrtc-p2p-lan') {
    return LAN_LEVEL_SPECS[level];
  }
  // TURN / Internet / Binary WS / unknown: Internet-safe base specs
  return internetSpec(level === 'ultra' ? 'high' : level);
}

/**
 * Resolve width/fps/bitrate for an encode level under the active network path.
 * Optional fpsOverride is clamped to path-safe options.
 */
export function resolveEffectivePreset(
  level: LiveViewEncodeLevel,
  path: LiveViewNetworkPath,
  cfg: LiveViewProfileConfig = DEFAULT_LIVE_VIEW_PROFILE_CONFIG,
  fpsOverride?: number | null
): EffectiveLiveViewPreset {
  const safeLevel =
    level === 'ultra' && !isUltraAllowed(path) ? 'high' : level;
  const spec = specForPath(safeLevel, path);
  const pathMax = maxFpsForPath(path, cfg);
  let fps = Math.min(spec.fps, pathMax);
  if (fpsOverride != null && Number.isFinite(fpsOverride)) {
    fps = clampFpsForPath(fpsOverride, path, cfg);
  } else if (path === 'webrtc-p2p-lan' && safeLevel === 'medium' && fpsOverride == null) {
    fps = clampFpsForPath(cfg.lanDefaultFps, path, cfg);
  } else {
    fps = clampFpsForPath(fps, path, cfg);
  }

  // Binary WS: never exceed ws max regardless of override mistakes
  if (path === 'binary-ws') {
    fps = Math.min(fps, cfg.wsMaxFps);
    fps = Math.max(fps, cfg.wsMinFps);
  }

  const intervalMs = Math.max(33, Math.round(1000 / fps));

  return {
    level: safeLevel,
    width: spec.width,
    jpegQuality: spec.jpegQuality,
    fps,
    intervalMs,
    maxBitrateBps: spec.maxBitrateBps,
    networkPath: path,
  };
}

export function effectivePresetFromInternet(
  level: LiveViewEncodeLevel
): LiveViewQualityPreset {
  const safe = level === 'ultra' ? 'high' : level;
  return LIVE_VIEW_QUALITY_PRESETS[safe];
}

/** Next FPS on the LAN Auto ladder (or null if at top of current ladder). */
export function stepLanAutoFps(
  currentFps: number,
  delta: -1 | 1
): number | null {
  const ladder = [...LAN_AUTO_FPS_LADDER];
  let idx = 0;
  let bestDist = Infinity;
  for (let i = 0; i < ladder.length; i++) {
    const d = Math.abs(ladder[i] - currentFps);
    if (d < bestDist) {
      bestDist = d;
      idx = i;
    }
  }
  const next = idx + delta;
  if (next < 0 || next >= ladder.length) return null;
  return ladder[next];
}

export function networkPathLabelKey(path: LiveViewNetworkPath): string {
  switch (path) {
    case 'webrtc-p2p-lan':
      return 'live.pathLan';
    case 'webrtc-p2p-internet':
      return 'live.pathInternet';
    case 'webrtc-turn':
      return 'live.pathTurn';
    case 'binary-ws':
      return 'live.pathBinary';
    default:
      return 'live.pathUnknown';
  }
}
